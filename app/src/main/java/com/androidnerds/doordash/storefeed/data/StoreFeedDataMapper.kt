...
import com.androidnerds.doordash.storefeed.data.remote.*
import com.androidnerds.doordash.storefeed.domain.Location
import com.androidnerds.doordash.storefeed.domain.Menu
import com.androidnerds.doordash.storefeed.domain.Store
import com.androidnerds.doordash.storefeed.domain.StoreFeed

class StoreFeedDataMapper(
    private val storesListDataMapper:
    NullableInputListMapper<NetworkStore, Store>
) : Mapper<NetworkStoreFeedResponse, StoreFeed> {

    override fun map(input: NetworkStoreFeedResponse): StoreFeed {
        return StoreFeed(
            input.numResults,
            input.isFirstTimeUser,
            input.sortOrder,
            input.nextOffset,
            input.showListAsPickup,
            storesListDataMapper.map(input.stores),
        )
    }
}
